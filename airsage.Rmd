...
ctpp_csvFile <- strsplit(ctpp_flowFile,"[/]")[[1]][2]
...
*** 
## 6. Review of Census Data (2014)
This section reviews census 2014 household, population, worker and employment data
...
map <- leaflet() %>%
  ...
  addTiles %>%
  addPolygons(
    data = acsShape, group = "acs_data",smoothFactor = 0.5,
    stroke = TRUE,  weight = 3, color = "black",opacity = 0.5,
    fill = TRUE, fillOpacity = 0.5,
    fillColor = ~pal_pop(ALAND)
  )  
...
map_hhPop <- map %>%
 ...
 addLayersControl(
  baseGroups = c("OSM","Stamen"),
  overlayGroups = c("Districts", "HH Density"),
  options = layersControlOptions(collapsed = FALSE)
 ) %>%

hideGroup(c( "Districts"))
map_hhPop
...
map_popDen <- map %>%
  ...
  addLayersControl(
  baseGroups = c("OSM","Stamen"),
  overlayGroups = c("Districts", "Pop Density"),
  options = layersControlOptions(collapsed = FALSE)
 ) %>%
...
```{r emp worker density, echo=FALSE}
...
map_EmpDen <- map %>% 
 ...
 addLayersControl(
  baseGroups = c("OSM","Stamen"),
  overlayGroups = c("Districts", "Emp Density"),
  options = layersControlOptions(collapsed = FALSE)
  ) %>% 
...
# Plot Worker densities along the corridor
map_workDen <- map %>% 
  addPolygons(
 ...
 addLayersControl(
  baseGroups = c("OSM","Stamen"),
  overlayGroups = c("Districts", "Work Density"),
  options = layersControlOptions(collapsed = FALSE)
  ) %>% 
